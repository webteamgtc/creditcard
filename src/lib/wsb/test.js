...
// const ENDPOINT = process.env.WSB_ENDPOINT;                 // e.g. https://sandbox-api-merchant.wasabicard.com
// const API_KEY  = process.env.WSB_API_KEY;                  // from Wasabi
// const PRIVATE_KEY_B64 = process.env.WSB_PRIVATE_KEY_B64;   // PKCS#8 DER (base64, no PEM)
// const WSB_PUBLIC_KEY_B64 = process.env.WSB_PUBLIC_KEY_B64; // X.509 SPKI DER (base64, no PEM)

// if (!ENDPOINT || !API_KEY || !PRIVATE_KEY_B64 || !WSB_PUBLIC_KEY_B64) {
//   throw new Error("WSB env vars missing (WSB_ENDPOINT, WSB_API_KEY, WSB_PRIVATE_KEY_B64, WSB_PUBLIC_KEY_B64).");
// }
...
// // === Java-equivalent: SHA256withRSA (PKCS#1 v1.5) over *raw JSON string* ===
...
// function verifyRawBody(rawBodyString, signatureB64) {
...
// /**
//  * POST helper (Java parity)
//  * Signs request body with SHA256withRSA and verifies response the same way,
//  * both over the *raw* JSON string. Headers:
//  *   X-WSB-API-KEY, X-WSB-SIGNATURE
//  *
//  * @param {string} path  e.g. "/merchant/core/mcb/card/openCard"
//  * @param {object} payload  serialised as-is; {} if null/undefined
//  * @param {boolean} verifySig  default true; set false to skip verification
//  */
...
//   // IMPORTANT: read raw first (verify must use the exact bytes the server signed)
//   const rawText = await res.text();

//   // Verify only when provider says "success" (HTTP 200 is not enough for some APIs)
//   let json;
//   try {
//     json = rawText ? JSON.parse(rawText) : null;
//   } catch {
//     // If upstream ever returns non-JSON, surface it
//     throw new Error(`WSB non-JSON response: ${rawText}`);
//   }

//   if (verifySig && json?.code === 200) {
//     const respSig =
//       res.headers.get("X-WSB-SIGNATURE") || res.headers.get("x-wsb-signature");
//     const ok = verifyRawBody(rawText, respSig);
//     if (!ok) {
//       const err = new Error("WSB response signature verification failed");
//       // Attach response so callers can still inspect the payload if needed
//       err.response = json;
//       throw err;
//     }
//   }
